...
object LowerBound {
  ...
  val x: scala.collection.immutable.List[Product with fix.LowerBound.BoundObject[_ >: fix.LowerBound.Village with fix.LowerBound.Town with fix.LowerBound.City <: fix.LowerBound.Bound] with java.io.Serializable] = List(A, B, C)
}
