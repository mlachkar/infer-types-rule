...
object ExplicitResultTypesRefinement {
  ...
  val test: fix.ExplicitResultTypesRefinement.conflict with Product with java.io.Serializable = new conflict with Product with Serializable {
    override def productArity: Int = ???
    override def productElement(n: Int): Any = ???
    override def canEqual(that: Any): Boolean = ???
  }
  trait Chars { def chars: CharSequence }
  val chars: fix.ExplicitResultTypesRefinement.Chars{val chars: String} = new Chars {
